...
def shp2clip(originfig,ax,shpfile,region):
    ...
    sf = shapefile.Reader(shpfile, encoding='utf-8') # encoding utf-8 / ISO8859-1
    for shape_rec in sf.shapeRecords():
        if shape_rec.record[2] == region:  ####Here you need to find a unique identifier that matches the region, and one item in record[] must correspond.
            vertices = []
            codes = []
